...
public class PipeMeshBuilder extends PipePartBuilder {
    ...
    void drawPipe(float length) {
        for (int i = 0; i < 4; ++i) {
            Vec3d up = up();
            quad(up, pos.add(up.multiply(SIDE / 2)).add(right.multiply(SIDE / 2)),
                    pos.add(up.multiply(SIDE / 2)).subtract(right.multiply(SIDE / 2)).add(facing.multiply(length)));
            rotateCw();
        }
    }
    ...
    void noConnection() {
        Vec3d up = up();
        quad(facing, pos.add(up.multiply(SIDE / 2)).add(right.multiply(SIDE / 2)),
                pos.subtract(up.multiply(SIDE / 2)).subtract(right.multiply(SIDE / 2)));
    }
}

...
public class PipeMeshBuilder extends PipePartBuilder {
    ...
    /**
     * Add a quad with four corners and the facing direction. It is important that 1 and 4 be opposite corners!
     * UVs are actually (u, v, whatever)
     */
    private void quad(Vec3d facing, Vec3d[] corners, Vec3d[] uvs) {
        if(corners.length != 4 || uvs.length != 4) throw new RuntimeException("This is a bug, please report!");
        Vec3d c1 = corners[0];
        Vec3d c4 = corners[3];
        ...
        else if (direction == DOWN) quad(DOWN, x, z, X, Z, y);
    ...
    void drawPipe(float length, Intent intent, boolean end) {
        ...
        for (int i = 0; i < 4; ++i) {
            if(intent != Intent.STRAIGHT && i == 0) length -= SIDE;
            double u = cols[i];
            Vec3d up = up();
            ...
            quad(
                    up,
                    new Vec3d[] {
                            base.add(right.multiply(SIDE/2)),
                            base.subtract(right.multiply(SIDE/2)),
                            base.add(right.multiply(SIDE/2)).add(facing.multiply(length)),
                            base.subtract(right.multiply(SIDE/2)).add(facing.multiply(length)),
                    },
                    new Vec3d[] {
                            new Vec3d(u + COL_WIDTH, length, 0),
                            new Vec3d(u, length, 0),
                            new Vec3d(u + COL_WIDTH, 0, 0),
                            new Vec3d(u, 0, 0),
                    });
            rotateCw();
            if(intent != Intent.STRAIGHT && i == 0) length += SIDE;
        ...
        if(end) {
            ...
            quad(
                    facing,
                    new Vec3d[]{
                            base.subtract(up.multiply(SIDE / 2)).subtract(right.multiply(SIDE / 2)),
                    ...
                    intent == Intent.STRAIGHT ?
                            new Vec3d[] {
                                    new Vec3d(4 * COL_WIDTH, 0, 0),
                                    new Vec3d(3 * COL_WIDTH, 0, 0),
                                    new Vec3d(4 * COL_WIDTH, COL_WIDTH, 0),
                                    new Vec3d(3 * COL_WIDTH, COL_WIDTH, 0),
                            } :
                            new Vec3d[]{
                                    new Vec3d(COL_WIDTH, 1, 0),
                                    new Vec3d(0, 1, 0),
                                    new Vec3d(COL_WIDTH, 1 - COL_WIDTH, 0),
    ...
    private static final int[][] CENTER_PATTERNS = new int[][] {
            new int[] { 1, 0, 1, 0 },
            new int[] { 0, 1, 1, 0 },
            new int[] { 0, 0, 0, 0 },
            new int[] { 1, 1, 1, 1 },
    ...
    private static final double[][] CENTER_UVS = new double[][] {
            new double[] { 0, 0 },
            new double[] { COL_WIDTH, 0 },
            new double[] { 3*COL_WIDTH, 0 },
            new double[] { 3*COL_WIDTH, COL_WIDTH },
            new double[] { 3*COL_WIDTH, 2*COL_WIDTH },
        ...
        for(int i = 0; i < CENTER_PATTERNS.length; ++i) {
            ...
            rotations: for(int j = 0; j < 4; ++j) {
                ...
                Vec3d[] vertices = new Vec3d[] {
                        pos.add(right.multiply(SIDE / 2)).subtract(up.multiply(SIDE / 2)),
                ...
                Vec3d[] uvs = new Vec3d[] {
                        new Vec3d(u, v + COL_WIDTH, 0),
                        new Vec3d(u, v, 0),
                        new Vec3d(u + COL_WIDTH, v + COL_WIDTH, 0),

...
public class CreativeTankBlockEntity extends BlockEntity implements FluidExtractable, BlockEntityClientSerializable, RenderAttachmentBlockEntity {
    ...
    public boolean onPlayerUse(PlayerEntity player) {
        ...
        // Try to set fluid
        if(fluid.isEmpty()) {
            fluid = FluidAttributes.EXTRACTABLE.get(heldStackRef, excessConsumer).attemptExtraction(ConstantFluidFilter.ANYTHING, FluidAmount.ABSOLUTE_MAXIMUM, Simulation.SIMULATE).getFluidKey();
            onChanged();
